...
\]
Combined with $\#\nuxy{a}{d} \leq d-a$ (since $\seplistvar$ is representable), this yields $\#\nuxy{a}{d} = d-a$, qed.}. Hence, those among all such intervals that are maximal under inclusion are pairwise disjoint and separated from each other by at least a distance of $1$.
We summarize our observations in the follwing lemma:
\begin{lemma}
\label{lemma:irreducible}
 ...
 \begin{enumerate}
  ...
  \item The set $\bigcup_{k=0}^{r}[b_k,a_{k+1})$ is minimal by inclusion among all sequences $(1=b_0\leq a_1<b_1<a_2<b_2<\dots<a_r<b_r\leq a_{r+1}=\lambda_1+1)$ satisfying property 1.
 \end{enumerate}
