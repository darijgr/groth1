...
Note that these notions depend on the skew-shape, so when we want to use a skew-shape $\widetilde\lm$ rather than $\lm$, we will write that $\cutvar$ is non-representable/irreducible/etc \textit{with respect to $\widetilde\lm$}, and we denote the corresponding partitions by $\nuab^{\widetilde{\lm}}$.
...
\label{lemma:irreducible}
 ...
 \end{itemize}
 where $\lm\big|_{[b_k,a_{k+1})}$ is a skew-shape $\lm$ with columns $1,2,\dots,b_k-1,a_{k+1},a_{k+2},\dots$ removed.
\end{lemma}\qed
\begin{definition}
