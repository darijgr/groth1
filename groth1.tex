...
\def\OneTwoRPP{{\operatorname*{RPP}\nolimits^{12}\left(  \lambda/\mu\right)}}
\def\BenignTables{{\operatorname*{BT}\nolimits^{12}\left(  \lambda/\mu\right)}}
...
\begin{definition}
 \label{defi.flip}
For a 12-table $T$, we put $\flip(T)$ to be the 12-table obtained from $T$ by changing each column of $T$ as follows:
...
Now the map $\B$ is defined as follows: take any 12-rpp $T$ and apply flip to it to get a 12-table $\flip(T)$. Next, apply descent-resolution steps to $\flip(T)$ in arbitrary order until we get a 12-table with no descents left. Put $\B(T):=P$ (a rigorous version of this is Definition \ref{defi.B}).
...
Let $\lm$ be a skew partition. We shall keep $\lm$
fixed for the rest of Section \ref{sect.proof}. 
...
It is clear that $\operatorname*{flip}$ is an involution on $\BenignTables$ that preserves $\operatorname*{ceq}$ and $\operatorname*{seplist}$ but acts as a transposition $s_1$ on $\operatorname*{ircont}$.
\end{remark}
...
Now, we have
\[
\left\{  N\in\BenignTables\ \mid\ T\overset{\ast}{\Rrightarrow}N\right\}
=\left\{  T\right\}  \cup\bigcup_{Z\in\mathbf{Z}}\left\{  N\in\BenignTables%
\ \mid\ Z\overset{\ast}{\Rrightarrow}N\right\}  .
\]
Intersecting both sides of this identity with the set $\OneTwoRPP$, we obtain%

\begin{comment}

\begin{align*}
\left\{  N\in\OneTwoRPP\ \mid\ T\overset{\ast}{\Rrightarrow}N\right\}    &
=\underbrace{\left(  \left\{  T\right\}  \cap\OneTwoRPP\right)  }%
_{\substack{=\varnothing\\\text{(since }T\notin\OneTwoRPP\text{)}}}\cup
\bigcup_{Z\in\mathbf{Z}}\underbrace{\left\{  N\in\OneTwoRPP\ \mid
...
Thus,
\begin{equation}
\operatorname*{Norm}\left(  T\right)  =\left\{  N\in\OneTwoRPP\ \mid
...
\begin{equation} 
\operatorname*{Norm}\left(  T\right)  =\left\{  N\in\OneTwoRPP\ \mid\ T\overset{\ast}{\Rrightarrow}N\right\} 
...
\par
But $B\overset{\ast}{\Rrightarrow}D$. Thus, every $N\in\OneTwoRPP$ satisfying
...
Let $T\in\OneTwoRPP$. We define $\mathbf{B}\left(
...
$\mathbf{B}:\OneTwoRPP\rightarrow\OneTwoRPP$ in Lemma \ref{lem.BK} (for a
...
\item Having constructed this map $\mathbf{B}:\OneTwoRPP\rightarrow\OneTwoRPP%
$, we can construct the involution $\mathbf{B}_{Z}:\OneTwoRPP_{Z}%
\rightarrow\OneTwoRPP_{Z}$ in Lemma \ref{lem.BKi} (for a given finite convex
...
we apply the involution $\mathbf{B}:\OneTwoRPP\rightarrow\OneTwoRPP$ to this
...
constructed from the maps $\mathbf{B}:\OneTwoRPP\rightarrow\OneTwoRPP$ for
...
connected as a subgraph of $\Z^2$. This is a harmless assumption,
...
\textbf{(b)} Let $\nu=(i_1\geq\dots\geq i_s>0)$. If $\supp(i_r)\subset [a_k,b_k)$ for some $k$ then $i_r$ appears in exactly one of the concatenated partitions, namely, $\nuxy{a_k}{b_k}$. Otherwise there is an integer $k$ such that $\supp(i_r)\cap [b_k,a_{k+1})\neq\emptyset$. It remains to show that such $k$ is unique, that is, that $\supp(i_r)\cap [b_{k+1},a_{k+2})=\emptyset$. But the interval $[a_{k+1},b_{k+1})$ is non-empty, therefore there is an element $i$ in $\nu$ with $\supp(i)\subset [a_{k+1},b_{k+1})$. It remains to note that we get a contradiction: we get two numbers $i,i_r$ with $\supp(i_r)$ being both to the left and to the right of $\supp(i)$. 

\textbf{(c)}
To verify the last claim, we need to show that if $\left[c,d\right)$ is a nonempty interval contained in $\left[b_k, a_{k+1}\right)$ for some $k$, then $\#\nupxy{c}{d}^{J} < d-c$, where $J$ denotes the restricted skew partition $\lm\big|_{[b_k,a_{k+1})}$, and where $\seplistvar^{\prime} = \nuxycap{b_k}{a_{k+1}}$. If $c> b_k$ and $d< a_{k+1}$ then this follows from $\nuxy{c}{d}<d-c$, so suppose $c=b_k$ and $\#\nupxy{c}{d}^{J} = d-c$. This immediately implies that $\#\nuxy{a_k}{d}=d-a_k$ which contradicts the minimality of $[b_k,a_{k+1})$. The case $d=a_{k+1}$ is completely analogous.
\end{proof}
...
\label{cor:confluence}
 If $\seplistvar$ is an irreducible partition, then no matter in which order one resolves descents in $\flip(T)$, the resulting tableau $T'$ will be the same. The map $T\to T'$ is the unique Bender-Knuth involution on $\OneTwoRPPCutvar$.
\end{corollary}
...
Finally, notice that for a general representable partition $\seplistvar$ descents may only occur inside each irreducible component independently, so we conclude the chain of corollaries by stating that our constructed involutions are canonical in the following sense:
