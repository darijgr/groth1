...
$\operatorname*{seplist}T$ is weakly decreasing. 
...
\begin{itemize}
\item If the column is 1-pure, we replace all its entries by $2$'s.

\item If the column is 2-pure, we replace all its entries by $1$'s.

\item Otherwise we do not change it.
\end{itemize}

Once these transformations are made, the resulting filling of $Z$
is a 12-table which is still benign. We define $\operatorname*{flip}\left(
...
%EndExpansion
\]
then $\operatorname*{flip}\left(  T\right)$ is
\[
%TCIMACRO{\TeXButton{Y}{\ytableausetup{notabloids}
%\begin{ytableau}
%\none& 1 & 2 & 1 & 2 \\
...
$k$-th and the $\left(  k+1\right)  $-th columns of $P$ cannot both be mixed because $P$ is benign. Thus we introduce the following notations:
