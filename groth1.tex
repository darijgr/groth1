...
bars). We shall refer to this observation as the
``six-cells property''.
...
%EndExpansion
%$.
where $\operatorname*{sep}\nolimits_{v-1}B = s$ (that is, the
separating line in the $\left(v-1\right)$-th column of $B$ is at
the same height as the separating line in the $v$-th column of $A$).
Now, $v$ is a descent of $B$ (due to the six-cells property).
...
(\ref{pf.prop.BK.norm.indhyp}), applied to $S=D$). Its unique element
clearly lies in both $\operatorname*{Norm}\left(  B\right)  $ and
$\operatorname*{Norm}\left(  C\right)  $ (since
the relation $\overset{\ast}{\Rrightarrow}$ is transitive). Hence,
