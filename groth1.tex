...
[PG] I'm not sure what to write here so that it does not intersect with the end of the next paragraph.
\end{todo}
...
sketch the proof that our involution is a generalization of the latter. Finally, in Section \ref{sect.structure} we give an explicit formula for $\g_\lm(x_1,x_2,0,0,\dots;\t)$ which shows that the map that we constructed in Section \ref{sect.def} is the unique involution that shares certain natural properties with the classical Bender-Knuth involutions.
...
\begin{todo}
 ...
 %[PG] You're right; moved them to section 3.3
\end{todo}
 
 Examples of an rpp, a non-rpp and of a semistandard tableau can be found on Figure \ref{fig:fillings}.
...
More explicitly, we have thus shown that every nonempty interval $\left[a,b\right) \subseteq \left[1, \lambda_1+1\right)$ satisfying $\#\nuab = b-a$ splits our problem into two independent subproblems. But if two such intervals $\left[a,b\right)$ and $\left[c,d\right)$ satisfy $a\leq c\leq b\leq d$ then their union $[a,d)$ is another such interval, because in this case we have $\#\nuxy{a}{d} \geq \#\nuab + \#\nuxy{c}{d} - \#\nuxy{c}{b}$, but $\nuxy{c}{b}\leq b-c$ by representability of $\nu$.
\begin{comment}
