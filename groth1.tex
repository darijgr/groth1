...
%TCIDATA{LastRevised=Wednesday, June 03, 2015 01:28:15}
...
_{i}$ is the number of columns of $T$ in which the entry in the $i$-th row
equals the entry in the $\left(  i+1\right)  $-th row.) We call
...
\label{eq.gtilde.def}%
\end{equation}
It is clear that $\widetilde{g}_{\lambda/\mu}$ is a well-defined formal power
series in $\mathbf{k}\left[  \left[  x_{1},x_{2},x_{3},\ldots\right]  \right]
...
Let $\left(  t_{1},t_{2},t_{3},\ldots\right)  =\left(  1,1,1,\ldots\right)  $.
Then, $\mathbf{t}^{\alpha}=\left[  \alpha=\varnothing\right]  =\left[
\left\vert \alpha\right\vert =0\right]  $ for every weak composition $\alpha$.
Hence, if $T$ is an rpp of shape $Y\left(  \lambda/\mu\right)  $, then%
\begin{align*}
...
\text{ is the same thing as an}\\
\text{rpp of shape }Y\left(  \lambda/\mu\right)  \text{ which has no stagnant
cells}%
\end{array}
\right) \\
...
\end{equation}
(Here, $s_{i}\cdot\operatorname*{ircont}\left(  T\right)  $ means the result
of the transposition $s_{i}=\left(  i,i+1\right)  \in\mathfrak{S}_{\left(
\infty\right)  }$ acting on the weak composition $\operatorname*{ircont}%
\left(  T\right)  $.)
\end{theorem}
...
Before we prove this lemma, let us show how Theorem \ref{thm.BK} can be
...
of $\Phi\left(  T\right)  $ are precisely the stagnant cells of $T$. Thus, for
every $i\in\mathbb{N}_{+}$, we have $\left(  \operatorname*{ceq}\left(
\Phi\left(  T\right)  \right)  \right)  _{i}=\left(  \operatorname*{ceq}%
\left(  T\right)  \right)  _{i}$ (because of how $\left(  \operatorname*{ceq}%
\left(  \Phi\left(  T\right)  \right)  \right)  _{i}$ and $\left(
\operatorname*{ceq}\left(  T\right)  \right)  _{i}$ were defined in terms of
stagnant cells). In other words, $\operatorname*{ceq}\left(  \Phi\left(
T\right)  \right)  =\operatorname*{ceq}\left(  T\right)  $. This proves
(\ref{pf.lem.BKi.ceq}).} and%
\begin{equation}
...
&  =\left(  \operatorname*{ircont}\left(  \Phi\left(  S\right)  \right)
\right)  _{i}=0\ \ \ \ \ \ \ \ \ \ \left(  \text{by (\ref{pf.lem.BKi.ircont3}%
...
$Y\left(  \lambda/\mu\right)  $. Let us denote this subset by $Z$, and refer
to it as the $\left(  i,i+1\right)  $\textit{-domain} of $T$. Let
$\mathbf{R}_{Z}$ denote the set of all rpps of shape $Z$ whose entries all
belong to the set $\left\{  i,i+1\right\}  $. Lemma \ref{lem.BKi} \textbf{(b)}
yields that there exists an involution $\mathbf{B}_{Z}:\mathbf{R}%
_{Z}\rightarrow\mathbf{R}_{Z}$ (defined canonically in terms of $Z$) which has
the following property: For every $P\in\mathbf{R}_{Z}$, the equalities
(\ref{eq.lem.BKi.ceq}) and (\ref{eq.lem.BKi.ircont}) hold. Consider this
involution $\mathbf{B}_{Z}$. We have $Z=T^{-1}\left(  \left\{  i,i+1\right\}
\right)  $, and thus $T\mid_{Z}$ is an rpp of shape $Z$ whose entries all
belong to the set $\left\{  i,i+1\right\}  $. In other words, $T\mid_{Z}%
\in\mathbf{R}_{Z}$. Hence, the involution $\mathbf{B}_{Z}:\mathbf{R}%
_{Z}\rightarrow\mathbf{R}_{Z}$ gives rise to a new such rpp $\mathbf{B}%
_{Z}\left(  T\mid_{Z}\right)  $. Now, we define a new filling $T^{\prime}$ of
$Y\left(  \lambda/\mu\right)  $ as follows:%
\[
...
state this definition as follows: For every $T\in\operatorname*{RPP}\left(
\lambda/\mu\right)  $, the filling $\mathbf{B}_{i}\left(  T\right)  $ is the
unique $T^{\prime}\in\operatorname*{RPP}\left(  \lambda/\mu\right)  $ such
that $T^{\prime}\mid_{Z}=\mathbf{B}_{Z}\left(  T\mid_{Z}\right)  $ and
$T^{\prime}\mid_{Y\left(  \lambda/\mu\right)  \setminus Z}=T\mid_{Y\left(
...
every $T\in\operatorname*{RPP}\left(  \lambda/\mu\right)  $. So let us fix a
$T\in\operatorname*{RPP}\left(  \lambda/\mu\right)  $. Let $Z$ be the $\left(
i,i+1\right)  $-domain of $T$. By the brief definition of $\mathbf{B}%
_{i}\left(  T\right)  $, we know that $\mathbf{B}_{i}\left(  T\right)  $ is
the unique $T^{\prime}\in\operatorname*{RPP}\left(  \lambda/\mu\right)  $ such
that $T^{\prime}\mid_{Z}=\mathbf{B}_{Z}\left(  T\mid_{Z}\right)  $ and
$T^{\prime}\mid_{Y\left(  \lambda/\mu\right)  \setminus Z}=T\mid_{Y\left(
\lambda/\mu\right)  \setminus Z}$. Thus, $\mathbf{B}_{i}\left(  T\right)
\mid_{Z}=\mathbf{B}_{Z}\left(  T\mid_{Z}\right)  $ and $\mathbf{B}_{i}\left(
T\right)  \mid_{Y\left(  \lambda/\mu\right)  \setminus Z}=T\mid_{Y\left(
\lambda/\mu\right)  \setminus Z}$. But the $\left(  i,i+1\right)  $-domain of
$\mathbf{B}_{i}\left(  T\right)  $ is again $Z$. Thus, the brief definition of
$\mathbf{B}_{i}\left(  \mathbf{B}_{i}\left(  T\right)  \right)  $ yields that
$\mathbf{B}_{i}\left(  \mathbf{B}_{i}\left(  T\right)  \right)  $ is the
unique $T^{\prime}\in\operatorname*{RPP}\left(  \lambda/\mu\right)  $ such
that $T^{\prime}\mid_{Z}=\mathbf{B}_{Z}\left(  \mathbf{B}_{i}\left(  T\right)
\mid_{Z}\right)  $ and $T^{\prime}\mid_{Y\left(  \lambda/\mu\right)  \setminus
Z}=\mathbf{B}_{i}\left(  T\right)  \mid_{Y\left(  \lambda/\mu\right)
\setminus Z}$. But this unique $T^{\prime}$ is $T$ (because we have $T\mid
_{Z}=\mathbf{B}_{Z}\left(  \mathbf{B}_{i}\left(  T\right)  \mid_{Z}\right)
$\ \ \ \ \footnote{this follows from $\mathbf{B}_{i}\left(  T\right)  \mid
_{Z}=\mathbf{B}_{Z}\left(  T\mid_{Z}\right)  $ since $\mathbf{B}_{Z}$ is an
involution} and $T\mid_{Y\left(  \lambda/\mu\right)  \setminus Z}%
=\mathbf{B}_{i}\left(  T\right)  \mid_{Y\left(  \lambda/\mu\right)  \setminus
Z}$\ \ \ \ \footnote{since $\mathbf{B}_{i}\left(  T\right)  \mid_{Y\left(
\lambda/\mu\right)  \setminus Z}=T\mid_{Y\left(  \lambda/\mu\right)  \setminus
Z}$}). Hence, $\mathbf{B}_{i}\left(  \mathbf{B}_{i}\left(  T\right)  \right)
$ must be $T$. We thus have proven $\mathbf{B}_{i}\left(  \mathbf{B}%
_{i}\left(  T\right)  \right)  =T$, and so we have shown that $\mathbf{B}_{i}$
is an involution.
...
/\mu\right)  $. Let $Z$ be the $\left(  i,i+1\right)  $-domain of $T$. Recall
that $\mathbf{B}_{i}\left(  T\right)  $ is the unique $T^{\prime}%
\in\operatorname*{RPP}\left(  \lambda/\mu\right)  $ such that $T^{\prime}%
\mid_{Z}=\mathbf{B}_{Z}\left(  T\mid_{Z}\right)  $ and $T^{\prime}%
\mid_{Y\left(  \lambda/\mu\right)  \setminus Z}=T\mid_{Y\left(  \lambda
/\mu\right)  \setminus Z}$. Thus, $\mathbf{B}_{i}\left(  T\right)  \mid
_{Z}=\mathbf{B}_{Z}\left(  T\mid_{Z}\right)  $ and $\mathbf{B}_{i}\left(
T\right)  \mid_{Y\left(  \lambda/\mu\right)  \setminus Z}=T\mid_{Y\left(
\lambda/\mu\right)  \setminus Z}$. Notice that $Z$ is the $\left(
i,i+1\right)  $-domain of $\mathbf{B}_{i}\left(  T\right)  $ (this follows
easily from the definition of $\mathbf{B}_{i}\left(  T\right)  $).
...
Recall that $Z=T^{-1}\left(  \left\{  i,i+1\right\}  \right)  $. Hence, the
entries of $T\mid_{Z}$ all belong to the set $\left\{  i,i+1\right\}  $, while
the entries of $T\mid_{Y\left(  \lambda/\mu\right)  \setminus Z}$ all belong
to $\mathbb{N}_{+}\setminus\left\{  i,i+1\right\}  $. Thus, if two cells $c$
and $d$ of $Y\left(  \lambda/\mu\right)  $ satisfy $T\left(  c\right)
=T\left(  d\right)  $, then either both $c$ and $d$ lie in $Z$ or both $c$ and
$d$ lie in $Y\left(  \lambda/\mu\right)  \setminus Z$. Thus, the stagnant
cells of $T$ are precisely the stagnant cells of $T\mid_{Z}$ and the stagnant
cells of $T\mid_{Y\left(  \lambda/\mu\right)  \setminus Z}$. Now, for every
$p\in\mathbb{N}_{+}$, the definition of $\left(  \operatorname*{ceq}\left(
T\right)  \right)  _{p}$ yields%
\begin{align*}
&  \left(  \operatorname*{ceq}\left(  T\right)  \right)  _{p}\\
&  =\left(  \text{the number of }j\in\mathbb{N}_{+}\text{ such that }\left(
i,j\right)  \text{ is a stagnant cell of }T\right) \\
&  =\underbrace{\left(  \text{the number of }j\in\mathbb{N}_{+}\text{ such
that }\left(  i,j\right)  \text{ is a stagnant cell of }T\mid_{Z}\right)
...
\right)  _{p}=\left(  \operatorname*{ircont}\left(  T\right)  \right)
_{s_{i}^{-1}\left(  p\right)  }. \label{pf.thm.BK.ircont.goal1}%
\end{equation}
...
^{-1}\left(  p\right)  =p$. The definition of $\left(  \operatorname*{ircont}%
\left(  T\right)  \right)  _{p}$ yields%
\begin{equation}
...
\]
Now, since $s_{i}^{-1}\left(  p\right)  =p$, we have $\left(
\operatorname*{ircont}\left(  T\right)  \right)  _{s_{i}^{-1}\left(  p\right)
}=\left(  \operatorname*{ircont}\left(  T\right)  \right)  _{p}=\left(
\operatorname*{ircont}\left(  \mathbf{B}_{i}\left(  T\right)  \right)
\right)  _{p}$. Hence, (\ref{pf.thm.BK.ircont.goal1}) is proven under the
assumption that $p\notin\left\{  i,i+1\right\}  $.
...
\mathbf{B}_{i}\left(  T\right)  \right)  \right)  _{p}=\left(
\operatorname*{ircont}\left(  T\right)  \right)  _{s_{i}^{-1}\left(  p\right)
}$. Let $q=s_{i}^{-1}\left(  p\right)  $. Then, $q\in\left\{  i,i+1\right\}  $
(since $p\in\left\{  i,i+1\right\}  $).
...
&  =\left(  \operatorname*{ircont}\left(  T\right)  \right)  _{s_{i}%
^{-1}\left(  p\right)  }\ \ \ \ \ \ \ \ \ \ \left(  \text{since }q=s_{i}%
^{-1}\left(  p\right)  \right)  .
\end{align*}
Thus, $\left(  \operatorname*{ircont}\left(  \mathbf{B}_{i}\left(  T\right)
\right)  \right)  _{p}=\left(  \operatorname*{ircont}\left(  T\right)
\right)  _{s_{i}^{-1}\left(  p\right)  }$ is proven. This completes our proof
of (\ref{pf.thm.BK.ircont.goal1}).
...
\section{\label{sect.diamond}A simple diamond lemma}
...
\textit{Newman's lemma} (see, e.g., \cite{BezCoq}).\footnote{In the general
version, the finiteness of $\mathbf{S}$ and the length-decrease hypothesis are
replaced by a requirement that there exist no infinite sequences $\left(
a_{0},a_{1},a_{2},\ldots\right)  \in\mathbf{S}^{\infty}$ such that every
$i\in\mathbb{N}_{+}$ satisfies $a_{i}\Rrightarrow a_{i+1}$. The proof of this
generalization is harder than that of Lemma \ref{lem.diamond}, and not
constructive. While there is a constructive reformulation of this
generalization (presented in \cite[Lemma 3.3]{BezCoq}), we do not have a use
for it in this paper.} (Some authors refer to Newman's lemma as the
\textit{diamond lemma}, but the latter name is shared with a different fact.)

\begin{todo}
Find more references for the diamond lemma? Our proof is already
self-contained enough, but more context could be nice.
\end{todo}
...
(\ref{pf.lem.diamond.lem1}).} We also recall that the relation $\overset{\ast
}{\Rrightarrow}$ is the reflexive-and-transitive closure of the relation
$\Rrightarrow$. Thus, the relation $\overset{\ast}{\Rrightarrow}$ is reflexive
and transitive and extends the relation $\Rrightarrow$. Also,%
\begin{equation}
...
rise to a poset of conflicts. Does this have any interesting properties? What
exactly happens to this poset when we resolve a conflict? What kinds of posets
...
posets of conflicts of 12-tables (though I am not sure if these 12-tables can
be chosen benign).
\end{todo}
...
\begin{todo}
Pavel's alternative definition and uniqueness?
\end{todo}
