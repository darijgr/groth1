% This is the version of groth1.tex submitted to the Electronic Journal of
% Combinatorics the 25th of November 2015, and subsequently edited after
% referee report 1.
...
and $2$.
  ...
  \bigskip\noindent \textbf{Keywords:} Bender-Knuth involutions, reverse plane partitions, dual stable Grothendieck polynomials, symmetric functions.
\end{abstract}

\section{Introduction}

\emph{Grothendieck polynomials} were introduced by Lascoux and Sch\"{u}tzenberger \cite{LasSch82} to model classes of structure sheaves of Schubert varieties in the $K$-theory of flag manifolds. \emph{Stable Grothendieck polynomials} were introduced by Fomin and Kirillov \cite{FomKir96} as stable limits of certain sequences of Grothendieck polynomials. In particular, the stable Grothendieck polynomials $G_\lambda$ represent classes of structure sheaves of Schubert varieties in the $K$-theory of the Grassmannian, and can be seen as $K$-theory analogues of Schur functions. Buch \cite{Buch02} gave a combinatorial formula for $G_\lambda$ in terms of set-valued tableaux and described a Littlewood-Richardson rule for their structure coefficients.
...
is the number of \textit{columns}
of $T$ containing the entry $i$. Note that we are counting columns
rather than cells here; thus, $g_{\lambda/\mu}$ is not homogeneous
in general. As proven in
\cite[\S 9.1]{LamPyl}, this power series $g_{\lambda/\mu}$ is a symmetric
function.
%Lam and Pylyavskyy connect the $g_{\lambda/\mu}$ to the
...
As a consequence, we observe that the involution constructed in
...
% (including some standard conventions
...
As usual, we often omit trailing zeroes when writing a weak
composition. For instance, the partition $(5,2,1,0,0,0,\ldots)$ can be also written as $(5,2,1,0)$ or $(5,2,1)$.
...
A \textit{skew partition} $\lambda/\mu$ is a pair $\left(\lambda, \mu\right)$ of partitions satisfying $\mu\subseteq\lambda$ (as subsets of the plane). In this case, we shall also often use the notation $\lambda/\mu$ for the set-theoretic difference of $\lambda$ and $\mu$; this difference is called the \textit{Young diagram} of $\lm$.
% We cannot *identify* the pair with the set-theoretic difference since this assignment fails to be injective.
 ...
 %\footnote{Fulton calls semistandard tableaux just ``tableaux'', but otherwise is consistent with most of our notation.}.)
...
power series in countably many indeterminates $x_1,x_2,x_3,\dots$ over $\Z$
that is invariant under permutations of $x_1,x_2,x_3,\dots$ with finite support
and that has the degrees of its monomials bounded from above. 
% A power series is said to be \textit{bounded-degree}
...
\textit{ircont statistic}) as the weak composition
$\ircont\left( T\right) = \left(r_1,r_2,r_3,\dots\right)$
where $r_i$ is the number of \emph{columns} of $T$
...
then we define the \textit{column equalities vector} (or, by way of
abbreviation, the \textit{ceq statistic}) of $T$ to be the weak composition
...
\end{lemma}
 ...
 \begin{itemize}
  ...
  \item Replace all entries $i$ by $1$ and all entries $i+1$ by $2$. We get a 12-rpp $T'$ of some smaller shape, which is still a skew partition.
  (The skew partition itself is not always uniquely determined, but its Young diagram is, which is all that matters to us here.)
%   \footnote{Fine print: It has the form $\lm$ for some skew partition $\lm$, but this skew partition $\lm$ is not always uniquely determined (e.g., $\left(3,1,1\right)/\left(2,1\right)$ and $\left(3,2,1\right)/\left(2,2\right)$ have the same Young diagram). But the involution $\mathbf{B}$ constructed in the proof of Lemma~\ref{lem.BK} depends only on the Young diagram of $\lm$, and thus the choice of $\lm$ does not matter.}.
  \item Replace $T'$ by $\B(T')$.
  \item In $\B(T')$, replace back all entries $1$ by $i$ and all entries $2$ by $i+1$.
...
$\operatorname*{sep}\nolimits_{5}T=2$, and there are no other $k$ for which $\operatorname*{sep}\nolimits_{k}T$ is defined;
thus, $\operatorname*{seplist}T=\left(  4,4,2\right)  $.
...
\begin{proposition}\phantomsection
\label{prop.flip}
\begin{enumerate}
\item[\textbf{(a)}] We have $\operatorname*{flip}\circ
\operatorname*{flip}=\operatorname*{id}$.

\item[\textbf{(b)}] Let $T$ be a benign 12-table. When $T$ is transformed into
...
\item[\textbf{(c)}] For every benign 12-table $T$, we have%
\begin{equation}
...
% $. (Note that $n$ is allowed to be $0$.)} [PG] this is a standard term: https://en.wikipedia.org/wiki/Reflexive_transitive_closure
...
% % [PG] replaced by ``by Lemma 5.8 applied to $A=T$.'' I don't think we need to use the fact that Norm(D) is a one-element set, just knowing that it's nonempty is enough. It is obviously nonempty. If you disagree, I suggest writing ``by Lemma 5.8 applied to $A=T$ together with ...'', but please, don't make it a footnote again.
% }
...
% (we can do this because each pure column contains only one entry, and thus each descent-resolution described above affects only one row), 
...
In this section, 
%we let $\kk$ be the polynomial ring $\Z\left[t_1, t_2, t_3, \ldots\right]$ in countably many indeterminates, and 
%not quite sure when \kk was something else --PG
...
% This also holds for any commutative ring $\kk$ and any $t_1, t_2, t_3, \ldots \in \kk$, since the case we have considered (where $t_1, t_2, t_3, \ldots$ are polynomial indeterminates over $\Z$) is universal.
% [DG] The previous line only makes sense if we define the $\g_\lm$ in the general setting. Since we currently don't, let's skip it.
...
\bibitem[Buch02]{Buch02}Anders Buch, \textit{A Littlewood-Richardson rule for the K-theory of Grassmannians}, Acta Math.\ 189 (2002), 37--78.
...
\bibitem[FomKir96]{FomKir96}Sergey Fomin, Anatol N.\ Kirillov, \textit{The Yang-Baxter equation, symmetric functions, and Schubert polynomials}, Discrete Math.\ 153 (1996), 1--3, 123--143.
...
\bibitem[GaGrLi16]{GaGrLi16}Pavel Galashin, Darij Grinberg, Gaku Liu,
...
\bibitem[LasSch82]{LasSch82}Alain Lascoux, Marcel-Paul Sch\"{u}tzenberger, \textit{Structure de Hopf de l'anneau de cohomologie et de l'anneau de Grothendieck d'une vari\'et\'e de drapeaux}, C.\ R.\ Acad.\ Sci.\ Paris Sr.\ I Math 295 (1982), 11, 629--633.
