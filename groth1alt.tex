...
defined).
Recall that $s_k$ is an involution; thus, $s_k^2 = \id$. Now,
$s_k \cdot \underbrace{b}_{= s_k \cdot a}
= \underbrace{s_k \cdot s_k}_{= s_k^2 = \id} \cdot a = \id \cdot a = a$,
so that $a = s_k \cdot b$.
It is now easy to see that the pair $\left(  k,k+1\right)  $ is an
...
,n\right\}  $ (since $b=s_{k}\cdot a$). Furthermore, for any $\left(
...
v\right)  =\beta$. Moreover, $a=s_{k}\cdot b$, so that $a_{\alpha}=\left(
s_{k}\cdot b\right)  _{\alpha}=b_{s_{k}^{-1}\left(  \alpha\right)  }=b_{u}$
(since $s_{k}^{-1}\left(  \alpha\right)  =u$ (since $\alpha=s_{k}\left(
u\right)  $)). Also, $a=s_{k}\cdot b$, so that $a_{\beta}=\left(  s_{k}\cdot
...
k,k+1\right)  $. We thus have $\left(  \alpha,\beta\right) \neq \left(
